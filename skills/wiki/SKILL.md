...
4. **结构化组织** — 七大分类：实体、概念、人员、决策、流程、源摘要、查询结果
...
│
├── entities/                      <- 实体页（模块、组件、服务、类、工具）
├── concepts/                      <- 概念页（设计模式、架构决策、最佳实践）
...
| 分类 | 目录 | 适用内容 | 示例 |
|------|------|----------|------|
| entity | entities/ | 模块、组件、服务、类、工具、系统 | hai-api-server, message-builder, sglang-engine |
| concept | concepts/ | 设计模式、架构原则、最佳实践 | rollback-workflow, deny-actions-pattern |
...
```bash
mkdir -p WIKI_DIR/{entities,concepts,people,decisions,processes,sources,queries}
```
...
| 分类 | 目录 | 说明 | 示例 |
|------|------|------|------|
| entity | entities/ | 模块、组件、服务、类等具体实体 | hai-api-server, message-builder |
| concept | concepts/ | 设计模式、架构决策、最佳实践 | rollback-workflow, deny-actions |
| person | people/ | 团队成员、专长领域、负责模块 | jeff-xu, alice-chen |
...
Wiki initialized at WIKI_DIR
   Directories: entities/, concepts/, people/, decisions/, processes/, sources/, queries/
...
对每个文件，用 Read 工具读取内容，然后提取以下 7 类知识：
...
2. **概念（Concepts）**：设计模式、架构决策、工作流 — 跨文件的抽象
   - 名称
   - 描述
   - 涉及的实体

3. **人员（People）**：文件中提到的团队成员、作者、负责人
   - 名称
   - 专长领域（从上下文推断）
   - 负责的模块/系统
...
## New Pages
### [entity|concept|person|decision|process] page-name
Title: ...
Tags: ...
Content: ...
Related: [[link1]], [[link2]]
...
Ingest complete
   ...
   New pages: X (entities: A, concepts: B, people: C, decisions: D, processes: E, sources: F)
   Updated pages: Y
...
Wiki Status
   Wiki: WIKI_DIR
   Pages: N (entities: A, concepts: B, people: C, decisions: D, processes: E, sources: F, queries: G)
